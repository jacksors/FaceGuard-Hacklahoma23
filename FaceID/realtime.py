...
def analysis(
    ...
    DeepFace.find(
        img_path="C:\\Users\\jacks\\OneDrive\\Documents\\Hacklahoma\\FaceID\\database\\Jackson\\2.jpg",
        db_path=db_path,
                    ...
                    for detected_face in detected_faces_final:
                        ...
                        if len(dfs) > 0:
                            ...
                            if df.shape[0] > 0:
                                ...
                                print(candidate)
                                if candidate["Facenet_cosine"] < 0.4:
                                    ...
                                    authorized_users = ["Jackson", "Josh", "Joel"]
                                    
                                    try:
                                        print(Arduino.is_unlocked)
                                        if label in authorized_users and not Arduino.is_unlocked:
                                            if not Arduino.is_unlocked:
                                                det_thread = threading.Thread(target=detected, args=(label, custom_face))
                                                det_thread.start()
